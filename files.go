...
// WithProgress sets a callback to track the upload progress.
func WithProgress(fn ProgressCallback) UploadOption {
	return func(c *uploadConfig) {
		c.progressFunc = fn
	}
}

// WithRetry sets a callback to be notified when a chunk upload is retried.
func WithRetry(fn RetryCallback) UploadOption {
	return func(c *uploadConfig) {
		c.retryFunc = fn
	}
}
...
// UploadFile uploads a file to the specified folder using chunked, resumable uploads.
// The backend API expects multipart/form-data for each chunk.
func (s *FilesService) UploadFile(ctx context.Context, folderID int, filename string, r io.Reader, opts ...UploadOption) (*File, *http.Response, error) {
	config := &uploadConfig{
